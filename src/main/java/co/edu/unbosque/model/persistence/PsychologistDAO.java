...
public class PsychologistDAO implements CRUDoperation<PsychologistDTO>{
	...
	public PsychologistDAO() {
		...
		if(listPsychologists==null)listPsychologists=new ArrayList<>();
	}
	...
	public ArrayList<PsychologistDTO> loadData() {
		...
		try {
			...
			while (dbcon.getResultset().next()) {
				...
				data.add(new PsychologistDTO(userName, password, name, document, dateOfBirth, cityOfBirth, Year.of(yearOfGraduation), daysInService, suportedSessions, salary));
			}
			dbcon.close();
			return data;
		} catch (SQLException e) {
